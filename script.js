...
function displayCoffees(products) {
...
function SearchSetup()
{
   ...
   function filterCoffee(){
        const item = this.value.toLowerCase();
        const filtered = products.filter((p)=>
        {
          return  p.name.toLowerCase().includes(item)
        })
        displayCoffees(filtered);
    }

    searchBar.addEventListener('input', filterCoffee);
    searchButton.addEventListener('click', filterCoffee);
